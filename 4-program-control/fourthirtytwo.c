...
/**
 * main - (Modified Diamond-Printing Program)
 *
 * Modify the program you wrote in Exercise 4.31 to
 * read an odd number in the range 1 to 19 to specify the number
 * of rows in the diamond.
...
int main(void)
{
        ...
        for (int columns = 1; columns <= size_Diam; columns++) {
                for (int count_col = blank_spc; count_col > columns;
                     count_col--)
                {
                        putchar(32);
                }
